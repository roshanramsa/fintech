...
function SignupPage() {
  ...
  const handleSubmit = (e) => {
    ...
    // Get current health
    let currentHealth = parseInt(localStorage.getItem("health")) || 3;
    console.log("Current Health:", currentHealth); // Debugging log
  
    if (currentHealth > 0) {
      localStorage.setItem("health", (currentHealth - 1).toString());
      alert(`Login Submitted! Health decreased to ${currentHealth - 1}`);
    } else {
      alert("Health is already at 0! No more submissions allowed.");
    }
  };
  ...
  return (
    <>
        <div className="flex items-start justify-center h-screen bg-gradient-to-r from-pink-900 to-blue-900 bg-opacity-1">
            ...
            <img className="absolute top-10 left-10 w-70" src="./src/assets/fakeprag(1).png" alt="" />
